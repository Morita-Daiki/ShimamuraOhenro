...
cursor.execute('''CREATE TABLE IF NOT EXISTS stores
                (store_name TEXT, store_type TEXT, prefecture_name TEXT, full_address TEXT, phone_number TEXT)''')
...
for step_num in range(1, 111):  # Loop through steps 1 to 110
    ...
    try:
        ...
        for item in result_items:
            ...
            store_type, store_name = store_name.split(' ', 1)
            ...
            cursor.execute('SELECT * FROM stores WHERE store_name=? AND phone_number=? AND store_type=?',
                           (store_name, phone_number,store_type))
            existing_record = cursor.fetchone()

            if existing_record is None:  # If no match found, insert the store information
                cursor.execute('INSERT INTO stores VALUES (?, ?, ?, ?, ?)',
                               (store_name, store_type, prefecture_name, full_address, phone_number))
                conn.commit()
